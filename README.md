...
| | |
|---|---|
| **Nothing, if you have the packaged download** | `AIChess3D-windows.zip` contains the game and the small program that serves it, with Python folded inside. Unzip it, double-click `Play AI Chess3D.bat`, play. |
| **Python 3.8 or newer, if you run from source** | Only when you clone the repository instead. The game is served by a small Python program that uses the standard library and nothing else. [python.org/downloads](https://www.python.org/downloads/) — on Windows, tick **Add python.exe to PATH** on the first screen. |
...
the board opens in your browser. In the packaged download it starts
`AIChess3D.exe` and needs no Python. From a clone it uses Python, and if that is
missing the launcher says so and points you at the download rather than closing
in your face.
...
### Making the packaged download

Only whoever publishes the game does this; players never do.
...
That writes `dist/AIChess3D/` (the program, the launcher and the `web/` folder,
which stays as plain editable files) and `dist/AIChess3D-windows.zip` of the
same. It then starts the result and requests the page, the opening book and the
health check from it, so a broken package fails on your machine rather than
someone else's. Build on the operating system you are building for.
...
`tools/build-book.py`); once out of book it runs a real search — alpha-beta with
a hand-written evaluation — in a worker, so the board stays responsive while it
thinks. It lives in the page and needs no setup. While it is still following a
line, the move list names it.

**Difficulty** has six levels, and they differ in how much the engine is allowed
to think, not in how often it is made to blunder at random:

| Level | Looks ahead | Behaviour |
|---|---|---|
| 1 · Beginner | 1 move | Misses that its own pieces can be taken. Ignores the book. |
| 2 · Casual | 2 moves | Sees a capture and its recapture, nothing deeper. |
| 3 · Club player | 3 moves, plus captures | Sound at simple tactics, sometimes plays a slightly worse move. |
| 4 · Advanced | up to 7 moves, 1.5 s | Calculates ahead; rarely gives anything away. |
| 5 · Expert | up to 9 moves, 3 s | Plays the best line it finds. |
| 6 · Master | up to 20 moves, 7 s | Everything it has, every move. |

The lower levels are also shaken off the best move by a small random amount, the
way a weaker player misjudges a position. The higher ones are not. Strong levels
are limited by the clock, so they play deeper on a faster machine. For the two AI
opponents the level is passed along as an instruction; how closely a model follows
it is up to the model.
...
A theme is chosen by its **texture**: **Theme › Texture** on the setup screen, or
the texture menu beside the buttons in a game. A texture is an entry in
`web/themes.json`. It restyles the background, the board, the pieces and the
buttons together, and switching one mid-game keeps the position, because only the
materials change — no geometry is rebuilt.

### Your own colours

**Colours…** (setup) or the **◐ Colours** button (in a game) opens a panel with a
picker for the background, the light squares, the dark squares, the white pieces,
the black pieces and the buttons. They are applied *on top of* the texture and
change colour only: Marble stays polished stone and Neon stays lit glass whatever
you paint them, because gloss, glow, fog and lighting remain the texture's own.
Painting the dark squares carries the frame with them, and the background carries
the fog. The move highlights keep the texture's colours so a check or a capture
stays readable on any board.

Colours last for the visit. Tick **Remember these colours for this texture** and
that texture comes back in them every time; each texture keeps its own. **Reset to
texture colours** puts one back as shipped and forgets what was saved.

### The buttons

The buttons over the board are part of the texture too: each one ships a colour,
and text is chosen to read on it. They show a name beside the symbol; **Show text
on the buttons** in the Colours panel turns the names off, which puts them back to
plain symbols (hover still names them).
...
Add it to the `themes` array and reload. No code change. Give it a `"ui": { "button": "#…" }` so its buttons have a colour.
...
tools/build-book.py    regenerates web/openings.json (needs python-chess; the app does not)
tools/build-exe.py     packages the gateway so nobody needs Python (needs PyInstaller; players do not)
web/
  ...
  js/
    ...
    engines.js         the engine boundary, and the built-in opponent
    movegen.js         a fast move generator, for searching
    search.js          evaluation, search and the difficulty levels
    search.worker.js   runs the search off the page's thread
    hud.js             the 2D chrome
    themes.js          textures, and the player's colours on top of them
...
Tests in five suites, nothing mocked: the rules layer and the engine's move generator are checked
against perft counts, the piece set against its own geometry, and the gateway by
starting it as a real process and reaching it over real HTTP — or, with
`CHESS3D_EXE` set to the packaged program, the same tests against that. The last gateway
...
Free to download, use, modify and share — **not** free to sell, and not for use
inside anything that bills or takes money in any form. That is a source-available
licence, not an open-source one: see [LICENSE](LICENSE) for the exact terms. The
libraries in `web/js/vendor/` keep their own licences.
